...
import ch.ethz.asltest.Utilities.Statistics.Element.AccumulationDoubleElement;
import ch.ethz.asltest.Utilities.Statistics.Element.TimestampedElement;

public final class AverageDoubleStatistics extends WindowStatistics {
    private AccumulationDoubleElement windowElement = new AccumulationDoubleElement(0L, 0, 0.0);
    ...
    private void finishWindow(boolean useLastElement)
    {
        if (useLastElement) {
            windowAverages.add(new TimestampedElement(getWindow() - TIME_INTERVAL, lastElement));
        } else {
            windowAverages.add(new TimestampedElement(getWindow() - TIME_INTERVAL, windowElement.getAccumulated() / WINDOW_SIZE));
        }
    }
